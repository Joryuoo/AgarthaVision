...
- `SessionManager` — start, resume, pause/resume inference, restore and detach all read and
  write this row (`core/session/SessionManager.kt`). There is no end: nothing writes `ended_at`
  since 86d4ab4vm, though the column and its remote close path stay for rows that already have
  one.
