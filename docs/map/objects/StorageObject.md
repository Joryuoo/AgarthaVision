...
(`schema.ts:506-511`). What this repo defines is the four policy decisions in
...
`metadata`, `path_tokens`, `version` — are catalogued at `schema.ts:506-542`. Do not write any
of them directly.
...
`data/supabase/SyncSampleUseCase.kt:51-83`, `schema.ts:506-542`.
